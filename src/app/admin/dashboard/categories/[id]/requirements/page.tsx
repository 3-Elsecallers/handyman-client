...
export default function CategoryRequirementsPage() {
  const params = useParams();
  const router = useRouter();
  const categoryId = params.id as string;

  const [category, setCategory] = useState<ServiceCategory | null>(null);
  const [requirements, setRequirements] = useState<CategoryVettingRequirement[]>([]);
  const [questions, setQuestions] = useState<CategoryQuestion[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const [reqDialogOpen, setReqDialogOpen] = useState(false);
  const [editingRequirement, setEditingRequirement] = useState<CategoryVettingRequirement | null>(null);
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    setError(null);
    const [catRes, reqRes, qRes] = await Promise.all([
      getCategories(true),
    ...
    if (catRes?.status === 200 && catRes.data.data) {
      const found = catRes.data.data.find((c: { id: string }) => c.id === categoryId);
    ...
    if (!catRes || catRes.status !== 200) {
      setError(catRes?.data?.message || "Failed to load category.");
    }
    setLoading(false);
  }, [categoryId]);
  ...
  const requirementFormik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      if (editingRequirement) {
        response = await updateCategoryRequirement(editingRequirement.id, payload);
      } else {
        response = await createCategoryRequirement(categoryId, payload);
      }
  ...
  const questionFormik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      let response;
      if (editingQuestion) {
        response = await updateCategoryQuestion(editingQuestion.id, payload);
      } else {
        response = await createCategoryQuestion(categoryId, payload);
      }
  ...
  const handleOpenEditRequirement = (req: CategoryVettingRequirement) => {
  ...
  const handleOpenEditQuestion = (q: CategoryQuestion) => {
  ...
  const handleDelete = async () => {
    ...
    if (deletingItem.type === "requirement") {
      response = await deleteCategoryRequirement(deletingItem.id);
    } else {
      response = await deleteCategoryQuestion(deletingItem.id);
    }
  ...
  const requirementColumns: AdminTableColumn<CategoryVettingRequirement>[] = [
    { label: "Name", key: "name" },
    {
      label: "Type",
  ...
  const questionColumns: AdminTableColumn<CategoryQuestion>[] = [
  ...
  return (
    <Box>
      ...
      <Typography variant="h4" component="h1" sx={{ fontWeight: 700, mb: 3 }}>
        Category Requirements - {category?.name ?? "Loading..."}
      </Typography>
